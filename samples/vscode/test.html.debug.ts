...
type BindingContext<ViewModel = any, ParentContext extends BindingContext = any, Parent = any, Root = any, Ancestors extends [...any, Root] = [...any, any]> = RootBindingContext<ViewModel> | ChildBindingContextImpl<ViewModel, ParentContext, Parent, Root, Ancestors>
...
interface ChildBindingContextImpl<ViewModel, ParentContext extends BindingContext, Parent, Root, Ancestors extends BindingContext[]> {
...
type ChildBindingContext<ViewModel, Parent extends BindingContext> = Parent & ChildBindingContextImpl<ViewModel, Parent, Parent['$data'], Parent['$root'], [...Parent['$parents']]>
...
type BindingContextTransformation<T, ChildContext extends <Parent>(parent: Parent) => object> =
 ...
 */
type ChildBindingContextTransform<T extends (...args: any[]) => any, Data, ParentContext> =
	T extends (data?: Data, parentContext?: ParentContext) => infer R ? R : never

/** The parent binding context to child binding context transformation (the transformation function) */
type ChildBindingContextTransformation<Handler extends ControlFlowBindingHandler<any>> =
...
type BindingHandlerType<Handler extends BindingHandler<unknown>> = Handler extends BindingHandler<(infer U)> ? U : never;

type ReadonlyObservable<T> = (ko.ObservableFunctions<T> | ko.ComputedFunctions<T>) & { (): T }
...
type MaybeReadonlyObservableArray<T> = readonly T[] | ReadonlyObservableArray<T>
...
const context_1_vm: RootBindingContext<vm> = (undefined as any)
function transformation_binding_0($context: typeof context_1_vm) {
...
function transformation_binding_1($context: typeof context_1_vm) {
const { $parents, $root, $data, $rawData } = $context
function tmp() {
const { isVisible, myText, myWidth } = $context.$data
return myTest
...
function transformation_binding_2($context: typeof context_1_vm) {
const { $parents, $root, $data, $rawData } = $context
function tmp() {
const { isVisible, myText, myWidth } = $context.$data
...
const binding_0 = transformation_binding_0(context_1_vm)
const binding_1 = transformation_binding_1(context_1_vm)
const binding_2 = transformation_binding_2(context_1_vm)
