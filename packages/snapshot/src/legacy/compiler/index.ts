  ...
  return {
    ...
    async compile(documents: Document[], reporting: Reporting): Promise<void> {
      ...
      const tsCompilerHost =
        compilerHost.createTypescriptCompilerHost(compilerOptions);

      tsCompilerHost.getSourceFile = (
        fileName: string,
          ...
          for (const node of nodeQueue) {
            ...
            if (
              ...
              throw new Diagnostic(
                ...
                translations
                  .map((t) => t.binding.bindingHandler.name)
                  .join(", "),
              );
      ...
      for (const [filename, sourceFile] of sourceFiles) {
        const map = sources
          .find((source) => source.filename === filename)
          ?.builder.getContent().map;
        const consumer = map
