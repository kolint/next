...
export class SourceBuilder {
  ...
  public commit(name?: string): void {
    ...
    if (name) this.currentNode.name = name;
  ...
  private createScaffold(): string {
    // The actual file extension is ".d.ts", however some module resolutions require it to end with ".js".
    return [
      BANNER,
  ...
  private mapIdentifier(
    id: IdentifierNode<string>,
    transform?: (identifierName: string) => string,
  ) {
    const trans = transform ? transform(id.value) : id.value;
    return new SourceNode(
      id.location.first_line ?? null,
      id.location.first_column ?? null,
      this.markupFileName,
      trans,
    );
  }
  ...
  private mapExpression(expr: BindingExpression) {
    return new SourceNode(
      expr.loc.first_line ?? null,
      expr.loc.first_column ?? null,
      this.markupFileName,
      expr.text,
    );
  }
  ...
  private createImportStatements(
    _markupFileName: string,
  ): (string | SourceNode)[] {
    const document = this.document;
    const viewmodelImports = document.imports
      .map((importNode) => {
        const isNamespaceImport =
          importNode.importSymbols.length === 1 &&
          importNode.importSymbols[0]!.name.value === "*";
        const createMappedImportSymbol = (
          symbol: (typeof importNode.importSymbols)[0],
        ) =>
          symbol.name === symbol.alias
            ? new SourceNode(null, null, null, [
                this.mapIdentifier(symbol.name),
              ])
            : new SourceNode(null, null, null, [
                this.mapIdentifier(symbol.name),
                " as ",
                this.mapIdentifier(symbol.alias),
              ]);
        if (isNamespaceImport) {
          ...
          return [
            ...
            name,
            " from ",
            this.mapIdentifier(importNode.modulePath, (id) => `"${id}"`),
            "\n",
          ];
        } else {
          const names = importNode.importSymbols.map((symbol) =>
            createMappedImportSymbol(symbol),
          );
  ...
  public createBindinghandlerImports(bindingNames: string[]): void {
    ...
    const transforms = bindingNames.map(
      (handler) =>
        new SourceNode(null, null, this.markupFileName, [
          `"${handler}": BindingContextTransform<${handler}>\n`,
        ]),
    );
  ...
  public createRootBindingContexts(context: BindingContext): void {
    ...
    this.currentNode.add(
      `const ${context.id}: RootBindingContext<${typeOf}${context.typeAssertion ?? "ViewModel"}> = (undefined as any)\n`,
    );
  }
  ...
  public createContextTransformation(
    ...
    const transformation = new SourceNode(
      loc.first_line ?? null,
      loc.first_column ?? null,
      this.markupFileName,
      [
        ...
        "return ",
        this.mapExpression(binding.expression),
        "\n",
        "}\n",
        "const bindingTransform = getBindingContextFactory(",
        this.mapIdentifier(wrapIdentifier('"', binding.bindingHandler, '"')),
        ")\n",
  ...
  public emitContextDefinition(
    ancestorContextId: string,
    binding: Binding,
  ): void {
  ...
  public emitContextDefinition2(typeNode: TypeNode): void {
    const x = typeNode.vm;
    const type =
      x instanceof ChildContextNode
        ? this.mapIdentifier(x.type.ref)
        : "undefined";
    const definition = [
      `const ${typeNode.getContext().id}: ChildBindingContext<`,
      type,
      `, typeof ${typeNode.getParentContext().id}> = undefined\n`,
    ];
    this.currentNode.add(definition);
  }
}
