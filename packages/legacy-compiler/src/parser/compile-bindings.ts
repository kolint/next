...
import { Binding, BindingName, BindingExpression } from "./syntax-tree.js";
...
export class ImportStatement {
  ...
  public getValue(path: string): string {
    ...
    if (this.isDefault) {
      ...
      if (this.name !== "ViewModel") result += `type ViewModel = ${this.name}`;
    }
...
export function isParentBinding(binding: Binding): boolean {
  ...
  return ["foreach", "let", "with", "using"].includes(
    binding.bindingHandler.name,
  );
}
...
function parseDataBind(
  ...
  function isProperty(
    property: meriyah.ESTree.ObjectLiteralElementLike,
  ): property is PropertyNode {
    return (
      property.type === "Property" &&
      property.value.start !== undefined &&
      property.value.end !== undefined &&
      property.key.start !== undefined &&
      property.key.end !== undefined &&
  ...
  const properties = objectExpression.properties
    ...
    .map<Property>((node) => {
      ...
      if (asProperty) {
        ...
        expression = data.substring(
          (node.value.start ?? 0) - 2,
          (node.value.end ?? 0) - 2,
        );
