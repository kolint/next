import { Diagnostic, diagnostics } from "../diagnostic.js";
...
export function createDocument(
  ...
  for (const node of tokens) {
    if (node instanceof DiagNode) {
      if (node.enable) {
        if (node.keys.length) reporting.enableDiagnostics(node.keys);
        else {
          reporting.enableAllDiagnostics();
        }
      } else {
        if (node.keys.length) reporting.disableDiagnostics(node.keys);
        else reporting.disableAllDiagnostics();
      }
    ...
    switch (node.nodeType) {
          ...
          const bindings = node.bindings
            .map((bindingData) =>
              parseBindingExpression(
                filePath,
                reporting,
                bindingData.bindingText,
                bindingData.location,
              ),
            )
            .flat();
          if (bindings.length) {
            for (const b of bindings) allBindings.add(b.bindingHandler.name);
      ...
      case NodeType.End: {
        ...
        if (lastNode?.key !== node.key)
          throw new Diagnostic(
            filePath,
            diagnostics["unbalanced-start-end-tags"],
            lastNode?.loc,
          );
        break;
      }
