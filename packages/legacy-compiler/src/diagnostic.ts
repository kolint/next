...
function getTsDiagnostics(): TsDiagnostics[] {
  const Diagnostics = (ts as any).Diagnostics as Record<string, TsDiagnostics>;
...
export class Diagnostic {
  ...
  public constructor(
    ...
    if (typeof diagnostic === "string") {
      ...
      if (diagnostic in diagnostics) diag = diagnostics[diagnostic];
      else throw new Error(`${diagnostic} is not a valid diagnostics`);
      ...
      const key = tsDiagnostics.find(
        (diag) => diag.code === diagnostic.code,
      )?.key;
...
export function setProblemsSeverity(
  problems: Diagnostic[],
  diagnosticSeverity: { [key in string]?: Severity },
): Diagnostic[] {
...
export const diagnostics = (<K extends string>(
  ...
  "multiple-context-bindings": {
    code: prefix(1),
    message:
      "Only one binding-handler per element can create child contexts. Move [$0] into separate elements.",
  ...
  "multiple-context-generating-bindings": {
    code: prefix(15),
    message:
      "Multiple context generating bindings in same node are not allowed.",
  },
  "binding-unknown": {
    code: prefix(16),
